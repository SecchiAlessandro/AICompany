...
import ModeToggle from "@/components/workflow-builder/ModeToggle";
import AIWorkflowMode from "@/components/workflow-builder/AIWorkflowMode";
...
export default function WorkflowBuilderPage() {
  ...
  const { currentStep, setStep, answers, roles, reset } = useWorkflowBuilderStore();
  ...
  const handleSave = () => {
    const yaml = finalYaml || generateWorkflowYaml(answers, roles);
    const name =
      answers.q1
        ?.toLowerCase()
        .replace(/[^a-z0-9\s]/g, "")
